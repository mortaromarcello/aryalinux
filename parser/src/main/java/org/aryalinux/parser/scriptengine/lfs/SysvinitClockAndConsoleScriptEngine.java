...
public class SysvinitClockAndConsoleScriptEngine extends ScriptEngine {

	@Override
	public void generateScript(List<Step> steps, String outputDirectory)
			throws Exception {
		for (Step step : steps) {
			if (step.getSource().equals("usage.html")) {
				String commands = Util.reformat(step.getCommands());
				String inittab = LineExtractor.extract(commands,
						"cat > /etc/inittab << \"EOF\"", "EOF");
				String sysconfigClock = LineExtractor.extract(commands,
						"cat > /etc/sysconfig/clock << \"EOF\"", "EOF");

				step.setCommands(inittab + "\n\n" + sysconfigClock);
