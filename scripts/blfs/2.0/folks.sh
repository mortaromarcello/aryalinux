#!/bin/bash

set -e
set +h

export XORG_PREFIX="/usr"
export XORG_CONFIG="--prefix=$XORG_PREFIX --sysconfdir=/etc \
    --localstatedir=/var --disable-static"

. /etc/alps/alps.conf

#DEP:dbus-glib
#DEP:gobject-introspection
#DEP:libgee
#DEP:evolution-data-server
#DEP:telepathy-glib
#DEP:python3
#DEP:vala


cd $SOURCE_DIR

wget -nc http://ftp.gnome.org/pub/gnome/sources/folks/0.10/folks-0.10.1.tar.xz
wget -nc ftp://ftp.gnome.org/pub/gnome/sources/folks/0.10/folks-0.10.1.tar.xz


TARBALL=folks-0.10.1.tar.xz
DIRECTORY=`tar -tf $TARBALL | sed -e 's@/.*@@' | uniq `

tar -xf $TARBALL

cd $DIRECTORY

./configure --prefix=/usr --disable-fatal-warnings &&
make

cat > 1434987998815.sh << "ENDOFFILE"
make install
ENDOFFILE
chmod a+x 1434987998815.sh
sudo ./1434987998815.sh
sudo rm -rf 1434987998815.sh


 
cd $SOURCE_DIR
sudo rm -rf $DIRECTORY
 
echo "folks=>`date`" | sudo tee -a $INSTALLED_LIST