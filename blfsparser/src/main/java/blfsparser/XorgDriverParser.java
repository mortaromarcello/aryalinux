...
public class XorgDriverParser extends Parser {
	List<Parser> parsers = new ArrayList<Parser>();

	public XorgDriverParser(String name, String path, String subSection) throws Exception {
		super(name, path, subSection);
		Element theDocument = getDocument();
		Elements sections = theDocument.select("div.sect2");
		for (Element section : sections) {
			parsers.add(new Parser("", section, "x"));
		}
	}
	...
	public void parse() {
		...
		parsers.get(0).getCommands().add(0,
