...
read -p "Inserisci la partizione di root esempio /dev/sda7 : " ROOT_PART
read -p "Inserisci la partizione della home esempio /dev/sda3 : " HOME_PART
read -p "Inserisci l'username che verrà loggato di default : " USERNAME
read -p "Inserisci il livello per l'entrata boot nel sistema live: " LABEL
read -p "Inserisci il nome della iso che verrà generata: " OUTFILE
CREATE_ROOTSFS="y"
...
menuentry "$LABEL"{
  echo "Caricamento di AryaLinux in corso. Attendere prego..."
  linux /isolinux/vmlinuz quiet splash
  initrd /isolinux/initram.fs
}

menuentry "$LABEL Debug Mode"{
  echo "Caricamento di AryaLinux nella modalità di debug in corso. Attendere prego..."
