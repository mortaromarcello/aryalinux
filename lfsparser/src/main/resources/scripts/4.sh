...
export KERNEL_VERSION=`ls /sources/linux*.tar.xz | sed 's/\/sources\/linux-//g' | sed 's/.tar.xz//g'`
...
if ! grep "ccache" $BUILD_LOG
then
	cd $SOURCES
	tar -xf ccache-3.2.3.tar.bz2
	cd ccache-3.2.3
...
make firmware_install INSTALL_FW_PATH=/lib/firmware
...
tar -xf linux-firmware-master.tar.gz -C /lib/firmware
...
echo "lfs-svn-x86_64" > /etc/lfs-release
...
echo "Execute 5.sh by entering the following :"
echo "./5.sh"
