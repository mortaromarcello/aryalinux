...
sudo tee rootscript.sh << "ENDOFROOTSCRIPT"
tar -xf ../git-manpages-2.9.2.tar.xz \
...
sudo tee rootscript.sh << "ENDOFROOTSCRIPT"
mkdir -vp   /usr/share/doc/git-2.9.2 &&
tar   -xf   ../git-htmldocs-2.9.2.tar.xz \
      -C    /usr/share/doc/git-2.9.2 --no-same-owner --no-overwrite-dir &&
find        /usr/share/doc/git-2.9.2 -type d -exec chmod 755 {} \; &&
find        /usr/share/doc/git-2.9.2 -type f -exec chmod 644 {} \;
