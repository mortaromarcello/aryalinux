...
sudo tee rootscript.sh << "ENDOFROOTSCRIPT"
xmlcatalog --noout --add "rewriteSystem" \
           "http://docbook.sourceforge.net/release/xsl/<em class="replaceable"><code><version></em>" \
           "/usr/share/xml/docbook/xsl-stylesheets-<em class="replaceable"><code><version></em>" \
    /etc/xml/catalog &&
